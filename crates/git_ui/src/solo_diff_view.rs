...
use editor::{
    DiffStyleControls, Direction, Editor, EditorEvent, EditorSettings, PopOutSplitDiff, SplittableEditor,
    ToggleSplitDiff,
...
impl Render for SoloDiffStyleToolbar {
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        h_flex()
            ...
            .child(DiffStyleControls::new(editor_entity.clone()))
            .child(
                IconButton::new("solo-diff-pop-out", IconName::ArrowUpRight)
                    .icon_size(IconSize::Small)
                    .toggle_state(editor_entity.read(cx).lhs_popout_window().is_some())
